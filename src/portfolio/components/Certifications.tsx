          ...
          {CERTS.map((cert) => (
            ...
            >
              <div
                ...
                style={{ backgroundColor: `${cert.badgeColor}22`, color: cert.badgeColor }}
              >
                {CERT_ICONS[cert.id]}
              </div>
              <div>
                ...
                <p className="text-brand-muted text-xs mt-1">{cert.issuer}</p>
                {cert.year && (
                  <p className="text-brand-muted/60 text-xs mt-0.5">{cert.year}</p>
                )}
              </div>
              <div className="mt-auto">
                <span
                  className="px-2.5 py-1 rounded-full border text-xs font-semibold group-hover:opacity-100 transition-opacity"
                  style={{ color: cert.badgeColor, borderColor: `${cert.badgeColor}44`, backgroundColor: `${cert.badgeColor}15` }}
                >
                  {cert.issuerShort}
                </span>
              </div>
