...
function MetricCard({ metric, animate }: { metric: typeof METRICS[number]; animate: boolean }) {
  const count = useCountUp(metric.value, 1800, animate);
  return (
    <div className="flex flex-col items-center gap-2 px-6 py-8 flex-1 min-w-0">
      ...
      <div className="text-brand-muted text-xs text-center leading-snug">{metric.sublabel}</div>
      <div className="mt-1 text-brand-cyan/50" aria-hidden="true">
...
export function Metrics() {
  ...
  return (
    <section ref={ref} className="bg-brand-surface border-y border-brand-border" aria-label="Key performance metrics">
