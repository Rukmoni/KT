        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="flex flex-col gap-6 order-2 lg:order-1">
            ...
            <h2 className="text-xl sm:text-2xl font-semibold text-brand-violet leading-snug">
              IT Delivery Manager · {PERSON.subtitle}
            </h2>

            <p className="text-brand-muted text-base sm:text-lg leading-relaxed max-w-xl">
              {PERSON.tagline}{' '}Now consulting independently through{' '}
              <a href={PERSON.website} target="_blank" rel="noopener noreferrer" className="text-brand-violet hover:text-brand-violet-light transition-colors">
                Kuvanta
              </a>.
            </p>
          ...
          <div className="relative flex justify-center order-1 lg:order-2">
            <div className="relative w-64 h-64 sm:w-80 sm:h-80">
              ...
              <div className="relative w-full h-full rounded-full border-2 border-brand-violet/50 overflow-hidden bg-brand-surface flex items-center justify-center">
                {/* Swap /nagarajan.jpg once headshot is available */}
                <img
                  src={PERSON.headshot}
                  alt={`${PERSON.name} — ${PERSON.title}, Kuala Lumpur`}
                  width={320}
                  height={320}
                  loading="eager"
                  decoding="async"
                ...
                />
                <div className="nm-fallback absolute inset-0 hidden items-center justify-center" aria-label="NM initials placeholder">
                  <span className="text-6xl font-bold text-brand-violet select-none">{PERSON.initials}</span>
                </div>
              </div>
              {BADGES.map((b) => (
                <div key={b.label} className={`absolute ${b.pos} px-3 py-1.5 rounded-full bg-brand-surface border border-brand-border text-brand-text text-xs font-semibold whitespace-nowrap shadow-lg`}>
                  {b.label}
                </div>
              ))}
            </div>
